...
?>
<div class='options discuss-switch search-ph'><h1 class='search-heading'>

<?php
	echo __('Products for ','ph_theme') . '<span class="search-query">'. $s . '</span><span class="nr"> ' . $NumResults . ' ' . $r . '</span></h1></div>';  
	...
	if ( have_posts() ) : ?>
			...
			$yesterday = strtotime("-1 day", strtotime($localDateMidnight));
			...
			if ($stamp >= $localDateEpoch)
			  ...
			  if ($day != $day_check) {
			    ...
			    echo "<div class='timing'><span class='day'>". $date ."</span><span class='phdate'>" . get_the_date() . "</span></div>";
			    $day_check = $day;
			    $fday = false;
			  }
